# Movement Detection using SVM (Time + FFT Features)
...
def main():
    ...
    # --------------------------
    model = SVC(
        kernel="rbf",      # best for non-linear motion patterns
        C=10,
        gamma="scale",
    ...
    plt.figure()
    plt.imshow(cm)
    plt.title("Movement Detection – SVM (Time + FFT)")
    ...
    plt.savefig("confusion_matrix_svm_fft.png", dpi=300, bbox_inches="tight")
    plt.show()

    print("\nSaved plot as confusion_matrix_svm_fft.png")
